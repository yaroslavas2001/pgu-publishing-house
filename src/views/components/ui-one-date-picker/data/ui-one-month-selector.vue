...
@Options<UiOneMonthSelector>({
  props: {
    minimumDate: Date,
  },
  emits:["changeMonth","onNavigation"]
})
export default class UiOneMonthSelector extends Vue {
  @Prop({ default: new Date(), type: Date })         date: Date;
  @Prop({ default: null, type: Date })        minimumDate: Date;
  monthsasdf: Array<string> = [
    "янв",
    "фев",
    "мар",
    "апр",
 ...
 innerDay:number = null;
  ...
  setMonths() {
    var self = this;   
    // console.log(this.monthsasdf);
    // console.log(this);
     
    this.innerMonths = this.monthsasdf.map(function (item, index) {
      return {
        number: index,
        name: item,
