<template>
  <div class="date-range-selector" v-if="!linkedDatePickerName">
    <ui-day-selector
      v-if="innerCurrentView == 'days'"
      @onNavigation="NavigatorDatetime('months')"
      @changeDay="ChangeData('days', 'days')"
      @changeCurrentView="ChangeCurrentView"
      @firstDateSelected="firstDateSelected"
      :firstDateSelect="firstDateSelect"
      :allowedToShowView="allowedToShowView"
      v-model:date="innerDate"
      @update:date="$emit('update:date', $event)"
      v-model:dateSecond="this.dateSecond"
    ...
    <ui-month-selector
      ...
      @changeMonth="ChangeData('days', 'month')"
      @getMonth="getMonth"
      :date="innerDate"
      :maximumDate="maximumDate"
      :minimumDate="minimumDate"
    ...
    <ui-year-selector
      ...
      @changeYear="ChangeData('months', 'years')"
      :date="innerDate"
      @getYear="getYear"
      :maximumDate="maximumDate"
      :minimumDate="minimumDate"
...
import UiDaySelector from "@views/components/ui-date-range-selector/ui-day-selector.vue";
import UiMonthSelector from "@views/components/ui-date-range-selector/ui-month-selector.vue";
import UiYearSelector from "@views/components/ui-date-range-selector/ui-year-selector.vue";

@Options<UiDateRangeSelector>({
  watch: {
    maximumView(value) {
...
export default class UiDateRangeSelector extends Vue {
  ...
  firstDateSelect = null;
  firstDateSelected(data: any) {
    this.firstDateSelect = data;
  }
  ...
  month = null;
  getYear(year: number) {
    if (this.innerYear != year) {
      this.year = year;
    } else {
      this.year = this.innerYear;
    }
  }
  getMonth(month: number) {
    if (this.innerMonth != month) {
      this.month = month;
    } else {
      this.month = this.innerMonth;
    }
  }
  /**
